...
//        return var_dump(json_encode($request->location) );
        ...
        $cou=ModelsRequest::where('company_id',auth()->guard('company')->user()->id)->first();
        $coun=$cou->count;
        ...
        foreach($request->location as $location){
            ...
            if(ModelsRequest::where('company_id',auth()->guard('company')->user()->id)->count() > $coun){
                return response()->json([
                    'message' => 'العدد تجاوز الحد',
                ], 401);
            }else {
                $requests->save();
            }
